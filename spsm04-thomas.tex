...
It has been previously shown that Android devices do not receive security updates promptly when critical vulnerabilities\footnote{If malicious code exploits a critical vulnerability then it gains control of the device.} are publicly disclosed and hence, on average, \daMeanInsecurityPercNominal\ are exposed to known critical vulnerabilities~\cite{androidvulnerabilities.org}.
...
This means that the OS version information from \da\ is likely to be overestimating the prevalence of new OS versions and hence our results are likely to be conservative.
