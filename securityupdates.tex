...
\input{avostats}
\newcommand{\avo}{AVO}
\newcommand{\avovulnfoot}[1]{}%{\footnote{\url{http://androidvulnerabilities.org/vulnerabilities/#1}}}
...
\todo{Make that percentage automatically generated so that it is correct}
...
If an malicious code exploits a critical vulnerability then it gains control of the device.
...
These vulnerabilities affect all Android devices regardless of manufacturer, and as a result our selected vulnerabilities will dominate security analysis of Android.
...
\begin{description}
\item[creation] When a vulnerability was created in the source code.
...
This occurs when the information about the vulnerability is freely available to the public from an accepted and independent source and has been validated by security experts so that it has a risk rating.
...
Hence when considering a vulnerability from the point of view of an organisation which cares about Advanced Persistent Threats, the critical period starts with \textbf{discovery} and continues until \textbf{fix deployed}.
...
Unfortunately \textbf{discovery} is the hardest point to obtain concrete data on as discovery may happen multiple times independently and not all discoverers will report their discovery.
\textbf{Creation} and \textbf{introducing release} are the earliest points which could be used, but the risk is mostly latent until someone discovers them and therefore we do not use these dates.
...
The date of first \textbf{exploit} is a point when the risk is definitely high, but again a good adversary will not be detected when using such exploits.
The date of first \textbf{fix} is, assuming that the fix is deliberate, a point at which the vulnerability is known at least within the organisation performing the fix and frequently implies an earlier discovery and notification by a third party; we determine this date from the Android Open Source Project repository by examining the authored-on date of the fixing commit.
...
Therefore, in this paper we use the earliest known date from \textbf{discovery}, \textbf{exploit}, \textbf{disclosure} or \textbf{fix}. A breakdown of the type of date used is shown in Table~\ref{tab:andvulns}.
...
We describe three experiments, examining the vulnerability of Android devices in general (\S\ref{sec:exp:versionsecurity}, explore the upgrade cycle and vulnerability cycle of the ecosystem (\S\ref{sec:exp:android_ecosystem}), quantify the updates installed on particular devices (\S\ref{sec:exp:device_updates}).
...
Once \emph{zergRush} was discovered in October 2011 the most devices are recorded as \emph{insecure} (red) as they were exposed to that vulnerability.
...
From October 2011 until the discovery of \emph{APK duplicate file} in February 2013 the graph shows progressive improvement as devices are upgraded.
...
In July 2011 the \emph{exploid}\avovulnfoot{exploid_udev} and \emph{levitator}\avovulnfoot{levitator} vulnerabilities both affect most Android devices, slowly these are fixed as updates roll out and devices are replaced until in January 2013 a much smaller proportion of devices are affected by known vulnerabilities.
...
However what is perhaps more worrying is the long tail on the \emph{Gingerbreak}\avovulnfoot{Gingerbreak}, \emph{levitator}, \emph{exploid} and \emph{zergRush}\avovulnfoot{zergRush} vulnerabilities which are more dangerous root vulnerabilities (not requiring new APK installation) and which affect a significant proportion of devices years later.
...
Those graphs summarise data across all the devices, however one of the advantages of the \da\ data is that we can look at what happens to individual devices over time.
...
\begin{minipage}{0.49\columnwidth}
 ...
 \caption{Updates between different Android versions in \da}
...
\begin{subfigure}[b]{\columnwidth}
 ...
 \label{fig:da_api}
\end{subfigure}
\caption{Monthly Android API version}
\end{minipage}\hspace{0.02\columnwidth}
\begin{minipage}{0.49\columnwidth}
...
Hence removing this anomaly would have no effect on our conclusions.
...
For \emph{RageAgainstTheCage adb}\avovulnfoot{RageAgainstTheCage_adb} and \emph{keystore buffer}\avovulnfoot{keystore_buffer} vulnerabilities we do not have sufficient data to determine whether they fit one of our attack vectors as the former may require physical ADB access and the latter may be thwarted by the sandbox.
...
This reduces the time available for an attacker to exploit a vulnerability by looking at security updates disclosed in the source code repository from the period (\textbf{first fix}, \textbf{fix deployed}) to (\textbf{fixing release}, \textbf{fix deployed}), unfortunately this can still take months .
