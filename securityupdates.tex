...
\end{figure}
A simple update process would be for a developer, who wrote the program, to write an update and ship it to the user who installs it.
The process for Android is rather more complicated and is shown in Figure~\ref{fig:update_ecosystem}.
Android builds on various open source projects such as the Linux kernel, the OpenSSL and BouncyCastle cryptography libraries and various Apache Java libraries.
It also incorporates various drivers for different bits of hardware.
The Android platform is then built on top of those with kernel and userspace components by Google and the Open Handset Alliance. %TODO work out the exact arrangements.
Usually the code for each update is kept secret until after the update has been published.
Then the Manufacturers take stock Android and customise it with different themes, apps, drivers and so on and ship updates on, to the operator.
The Operator may then make further customisations and do further testing before shipping the update to the User.
...
This multi-stage multi-party release process increases the exposure of Android devices to vulnerabilities as it takes a long time for them to be patched.
