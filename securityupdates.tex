...
\vspace{-1em}
\begin{abstract}
Modern mobile operating systems combine up to three layers of security protection to prevent app malware from taking control of a device.
These are: the defences provided by the app store, additional checks performed at installation time on the device, and a protected execution environment to control the running app.
In this paper we show that only the app store is currently able to provide effective protection for the majority of Android users.
In contrast, a number of well-known, publicly disclosed exploits continue to allow apps to bypass installation checks or bypass runtime restrictions.
We quantify for the first time the vulnerability of Android devices in the wild and how long it takes for critical flaws to be fixed on consumer devices.
To do so, we analyse \da\ data from over \daOSYearsOfData\ years and \daNumOSDevices\ devices and found that, on average, \daMeanInsecurityPercTwosfNominal\ of devices were exposed to known privilege-escalation attacks.
% and only \daUpdatednessPercTwosfNominal\ of devices run the most recent version of Android.
Furthermore, devices apply \daUpdatesPerYearTwosfNominal\ updates each year, less than the critical vulnerability discovery rate of between \avoVulnsPerYearAllAndroidTwosfNominal\ and \avoVulnsPerYearTwosfNominal\ per year.
\end{abstract}
...
We have selected our focus for two reasons: no previous published work has quantified how vulnerable Android devices are to malware gaining root, and secondly the negative impact on the device owner is much higher for malware that gains root than malware which does not. 
Such malware can perform arbitrary actions on the device, and it is also impossible for the device owner to remove the malware reliably as even a factory reset does not guarantee the malware has been removed. 

This is an important problem: previous studies have shown that a substantial proportion of malware attempts to gain root. For example, in 2012, 40\% of malware for Android contained exploits designed to gain root~\cite{Zhou2012a}.
...
The second attack vector is to coerce an existing app into downloading and executing code at runtime.
...
In particular, we have not included any device manufacturer specific vulnerabilities, even when these are widespread (such as vulnerabilities in Qualcomm\footnote{Qualcomm is particularly good at publicly disclosing vulnerabilities and the patches that fix them. \url{https://www.codeaurora.org/projects/security-advisories/}} chipsets) because it is difficult to work out which devices are affected.
...
Modern mobile operating systems combine up to three layers of security protection to prevent app malware from taking control of a device.
These are: the app store, checks performed during app installation, and the app sandbox.
In this paper we have demonstrated that the app sandbox is the best place to prevent malware from gaining root.
However the Android sandbox is ineffective in the majority of cases: the latency in the security update process means that on average \daMeanInsecurityPercNominal\ of Android devices are exposed to known critical vulnerabilities which allow a malicious app to break out of the sandbox.
Furthermore, only \daUpdatednessPercNominal\ of devices run the latest version of Android and devices apply \daUpdatesPerYearNominal\ updates each year, less than the critical vulnerability discovery rate of between \avoVulnsPerYearAllAndroidNominal\ and \avoVulnsPerYearNominal.
...
Secondly, users should enable the Google Verify Apps feature if available. 
...
%Thanks to: Richard Clayton, the mobile security reading group, Anil Madhavapeddy and Oliver Chick for reading drafts;
