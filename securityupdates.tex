...
Android has \percMarketShare\ of the smartphone market and while the core development is controlled by Google there are about \daNumManufacturers\footnote{\label{foot:dadata}We computed this from the Device Analyzer~\cite{Wagner2013} data, see \S\ref{sec:android_update_process}.} manufacturers which make devices which run Android.
...
The proportion of Android devices in the Device Analyzer data which were running versions of Android known to have root vulnerabilities at the time is shown in Figure~\ref{fig:proportioninsecure}.
...
The database of vulnerabilities which we maintain at \texttt{androidvulnerabilities.org} contains \avoNumVulnerabilities\ vulnerabilities of which \avoNumVulnAllAndroid\ affect all android devices and \avoNumVulnSpecific\ are specific to particular devices or manufacturers.
...
The dates of discovery of vulnerabilities in the \texttt{androidvulnerabilities.org} is not uniform.
...
During the period in which Device Analyzer data was collected the date when a version of Android with the fix was observed on a Device Analyzer device is taken as the date that version was released, before that the release date as best as we can determine is used.
There is no canonical source of Android release dates, our best guesses and supporting references are available from \texttt{androidvulnerabilities.org}.
...
The Device Analyzer data contains data from \daNumDevices\ devices with a total of \daDeviceDays\footnote{Here we are only counting devices and days for which we have valid OS version data} device days, it contains data for more than \daMonths\ months for \daMonthsDevices\ devices.
...
While we have not tried to exploit these vulnerabilities on the devices in order to test whether the devices are actually vulnerable, we are confident that most devices will be if they are running a vesion known to be vulnerable. %TODO WHY?
...
The Android platform is then built on top of those with kernel and userspace components by Google. %TODO work out the exact arrangements.
...
Sometimes Manufacturers incorporate upstream open source project releases directly, and sometimes incorrectly -- for example including a broken daily build of sqllite in their release of Android
...
%TODO talk about update latency

The numbers of Devices (\daNumOSDevices), Operators (\daNumOperators) and Manufacturers (\daNumManufacturers) in Figure~\ref{fig:update_ecosystem} come from the Device Analyzer data.
Manufacturer and Operator counts were obtained by normalising the results reported by Android to Device Analyzer of the device manufacturer and active network operator.
...
This normalisation is not perfect and so these are overestimates on the Device Analyzer data but they are likely still underestimates as there will be some manufacturers and operators which are not included in the Device Analyzer data.

The number of updates received by devices (\daNumFullVersions) is the number of different full version strings observed in Device Analyzer.
The number of updates shipped by Google (\daNumSigOSVersions) is the number of Android versions reported in Device Analyzer which affected more than \daSigVersionPerc\ of devices for more than \daSigVersionDays\ days. %TODO this is the wrong number to be using
...
In July 2011 at the beginning of the Device Analyzer data the \emph{exploid}\footnote{\url{http://androidvulnerabilities.org/vulnerabilities/exploid_udev}} and \emph{levitator}\footnote{\url{http://androidvulnerabilities.org/vulnerabilities/levitator}} vulnerabilities both affect most Android devices, slowly these are fixed as updates roll out and devices are replaced until in January 2013 a much smaller proportion of devices are affected by known vulnerabilities.
However when in February 2013 the first APK signing vulnerability was found which affected all previous versions of Android and even in October 2013 most devices were still vulnerable. %TODO put percentages on these things to quantify them
\begin{figure}%[!b]
\centering
...
Those graphs summarise data across all the devices, however one of the advantages of the Device Analyzer data is that it allows us to look at what happens to individual devices over time.
Figure~\ref{fig:device_data} shows how the number of vulnerabilities affecting the \daNumDeviceDataDevices\ devices which have contributed the most days of data to Device Analyzer changes over time.
...
\begin{figure}
 ...
 \caption{Number of vulnerabilities affecting the top \daNumDeviceDataDevices\ devices by days of contribution in the Device Analyzer data}
...
Examining the way that the OS version distribution changes over time in Figure~\ref{fig:norm_os} shows some regularity to the behaviour.
Figure~\ref{fig:frvh_os_versions} is a plot of the proportion of of devices in Device Analyzer running each Android version or a later Android version (i.e. for 2.1.0 it includes all devices as they all run later versions of Android).
The data are all normalised to the number of days since the version was released and is smoothed by forcing it to be monotonically increasing.\footnote{While sometimes an older version becomes more popular after a later version has been released causing the values to actually decrease, mostly this was just noise or changes in the distribution of devices (e.g.\ the sudden arrival of 1000 devices from Bangladesh due to a study being conducted there) and this assumption makes the graph clearer.}
\begin{figure}
 \includegraphics[width=\columnwidth]{figures/frvh_os_versions}
 \caption{Proportion of devices running particular versions of Android or any later version}
...
\end{figure}
There is a general pattern to the data, Figure~\ref{fig:frms_os_versions} shows the mean and standard deviation of the same data.
It also shows the curve of best fit, which has a root mean squared error (RMSE) of \daOSCurveFitRMSE, the curve we chose is not a standard polynomial fit (3 degree polynomial fit gave a RMSE of \daOSCurvePolyRMSE) or a spline fit (RMSE of \daOSCurveSplineRMSE) as we wanted a better model of behaviour than would be given by simply fitting a curve.

The data appears to have an inverse exponential decay as it tends to 1 but with a slow start.
This gives us a $1 - e^{-x}$ component for the main behaviour and a $\frac{1}{x}$ component to provide a slow start.
The general form is then:
\begin{equation}
y = (s - \frac{1}{e^{dx}} + \frac{1}{cx + a})l
\end{equation}
However there are two constraints we can apply, $\underset{x\rightarrow\infty}{\mathrm{lim}} y\rightarrow 1$ and the curve passes through the origin.
This gives us $l = \frac{1}{s}$ and $a = \frac{1}{1-s}$.
\begin{equation}
y = (s - \frac{1}{e^{dx}} + \frac{1}{cx + \frac{1}{1-s}})\frac{1}{s}
\end{equation}
We get the parameters ????? for OS versions and ???? for API versions.
