...
The build string is `The user-visible version string', fortunately most (99.9\%)\todo{use a generated number} devices in the data have a build string of the form `x.y.z random string' and so it is possible to extract the Android version number.
...
Hence we use dates such as: when CVEs were registered, when fixes were committed to source repositories, or when exploits were committed to repositories; as the date first known about.\todo{We need to be more precise here}
...
While we have not tried to exploit these vulnerabilities\todo{ethics} on the devices in order to test whether the devices are actually vulnerable, we are confident that most devices will be if they are running a vesion known to be vulnerable. \todo{WHY?}
...
\begin{figure}
 ...
 Dotted arrows indicate flows where we do not know how many updates are being produced as we can't measure those flows directly as they are not public.\todo{Update this diagram}}
...
Android builds on various open source projects such as the Linux kernel, the OpenSSL and BouncyCastle cryptography libraries and so can include any of the versions of those projects, including those which fix security vulnerabilities in them.
There are \avoNumExternalProjects\ external projects in Android, contributing \avoTotalExternalLines\ lines of code.
We analysed the top \avoNumBigExternalProjects\ by lines of code (contributing \avoNumBigExternalLinesOfCode, \avoBigExternalLinesOfCodePerc\ of the total) and were able to automatically extract the versions of those projects included in different versions of Android for \avoNumBigExternalProjectsAnalysed.
...
Usually the code for each update is kept secret\todo{Can we quantify this keeping the code secret? Is it worth it?} until after the update has been published though through the Open Handset Alliance.
Manufacturers in the  alliance receive advanced copies in order to prepare handsets.
...
\todo{talk about update latency, quantify this in the text as per the figure, giving more explanation, use statistics from samsung-updates.com -> how many binaries are there per device? Do we know the variation between network operators?}
...
\todo{redo this section in light of related work and to fit into ecosystem discussion}
...
\begin{figure}
 ...
 \caption{Proportion of devices not updated to particular versions of Android or any later version\todo{make line of best fit thicker}}
