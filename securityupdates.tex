...
These vulnerabilities affect all Android devices regardless of manufacturer, and as a result our selected vulnerabilities will dominate security analysis of the Android ecosystem as a whole.
...
(iii) device manufacturer specific vulnerabilities (which are not in our analysis here) were easier to find and therefore attackers looking for vulnerabilities adjusted their focus.
...
We examine the vulnerability of the Android ecosystem as a whole (\S\ref{sec:exp:versionsecurity}, explore the upgrade cycle and vulnerability cycle of the ecosystem (\S\ref{sec:exp:android_ecosystem}), quantify the updates installed on particular devices (\S\ref{sec:exp:device_updates}).
...
For some vulnerabilities the required patch is not in AOSP. This is despite the fact that some device manufacturers have shipped builds containing the fix. This also made it difficult to determine whether a specific device was vulnerable or not.
...
However a sensitivity analysis showed that those two vulnerabilities make little difference to our results as they affect few devices.

We still do not know where new versions of Android come from. Over time newer versions of Android come to dominate, but quantifying the proportion due to new phone purchases verses the proportion due to updates of existing devices is unknown.
