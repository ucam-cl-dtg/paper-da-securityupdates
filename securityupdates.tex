...
\newcommand{\bouncyCastleNumVersions}{A}
\input{countversions}
%\newcommand{\opensslNumVersions}{B}
\newcommand{\apacheLibsNumVersions}{C}
\newcommand{\apacheLibsNumLibs}{E}
%\newcommand{\linuxNumVersions}{D}
...
The number of updates shipped by upstream open source projects is calculated by scraping the releases pages for those projects.
...
However what is perhaps more worrying is the long tail on the \emph{Gingerbreak}\footnote{\url{http://androidvulnerabilities.org/vulnerabilities/Gingerbreak}}, \emph{levitator} and \emph{exploid} vulnerabilities which are more dangerous root exploits (not requiring new APK installation) and which still affect a significant proportion of devices years later.
...
\begin{figure}
 \includegraphics[width=0.5\textwidth]{figures/device-data-all-security}
...
SEAndroid~\cite{Smalley2013} which is included in Android from version XXX~\cite{TODO} claimed to prevent some root vulnerabilities and to reduce the impact of others.
